...
1. **記憶體管理**  
使用 std::vector 可避免手動管理記憶體，透過虛擬解構子 virtual ~MinPQ() 確保在多型操作下能正確釋放衍生類別的資源。
...
1. 實驗測得的 $Height / \log_2 n$ 比例穩定落在 2.1~2.6 之間，符合隨機 $BST$ 平均高度為常數倍 $\log_2 n$ 的理論。 
2. 刪除功能經驗證能正確處理根節點的刪除，並維持 $BST$ 的中序排序特性。

## 申論及開發報告

1. **選擇遞迴的原因**

二元樹本身具有遞迴的結構定義，因此使用遞迴實作插入刪除及高度計算能使程式碼易於維護。
